...
- 36955a2: Fix `compareState` comparing object against itself instead of the other object
- 34ed3ed: Fix: prevent double `?` and `#` prefixes in `createHref`
- c006e82: Fix: handle async result of `NativeNavigation.pop()` in `go`/`goBack`
- 26dab37: Fix: copy listeners array before iterating in `FixedHistoryWrapper`
